...
(function($){
    ...
    jQuery.fn.tableGear = function(options){
        ...
        setDefaults('deletePrompt',   'Удалить эту строку?', options);
        setDefaults('noDataMessage',  '- Нет данных -',      options);
        setDefaults('addRowLabel',    'Добавить строку',        options);
        ...
        function initialize(el){
            ...
            if(options.addNewRows && form){
                ...
                table.after(addRow);
                if(!perRow) perRow = $('td.editable:visible', emptyRow).length;
            }

            $('input[type=text]', table).attr('autoComplete', 'off');
            el.addClass('activated table table-striped table-condensed table-bordered table-hover ');
        ...
        function initializeRow(row, rowIndex){
            ...
            if(!perRow) perRow = $('td.editable:visible', row).length;
            ...
            $('td', row).each(function(columnIndex){
                ...
                if(cell.hasClass('editable') && cell.is(':visible')){
        ...
        function removeRow(row){
            ...
            editableCells = [];
            $('td.editable:visible', tbody).each(function(){ editableCells.push(this); });
            sortRows();
        }
        ...
        function updateField(row, cell, input, span, column){
            ...
            if(key){
                ...
                // Insert other fields as they may not be blank here.
                var other = $('td.editable input', row).not(input);
                other.each(function(o) {
                    params.data[getInputField($(this))] = this.value;
                });
            }

            $.post(url, params, function(response){
                jobComplete(response, row, cell, input, span, value);
            });
        ...
        function updateField2(row, cell, input, span, column){
        ...
        function jobComplete(response, row, cell, input, span, value){
            var json = $.parseJSON(response);
            ...
            var el = $('input[name="data['+json.key+']['+json.field+']"]');
            el.siblings('span').html(json.formatted);
            el.val(json.value);
            el.data('currentValue', json.value);
            if(json.totals){
        ...
        function sortRows(){
            ...
            $.each(rows, function(index){
                ...
                $('td.editable:visible', row).each(function(){
                    editableCells.push(this);
                });
                tbody.append(row);
            });
        ...
        function getDate(s){
            ...
            var dateInTable = s.split('.');
            var date = new Date(dateInTable[2], dateInTable[1] - 1, dateInTable[0]);
            if(!date.getYear()){
