...
use App\MainBundle\Extension\AjaxError;
use App\MainBundle\Extension\AjaxSuccess;
use App\MainBundle\Extension\TableGear;
use App\MainBundle\Form\CategoryType;
use App\MainBundle\Form\ProductType;
...
class AdminController extends Controller
{
	...
	public function categoryChildsAction()
	{
		...
		if (!empty($category)) {
			...
			if (!empty($categoryChilds)) {
				...
				$categoryChildrensHtml = $this->renderView('AppMainBundle:Admin:ul.childs.category.html.twig',
					array(
						'category' => $category,
	...
	public function categoryProductsAction()
	{
		...
		if (!empty($category)) {
			$categoryProductsHtml = $this->renderView('AppMainBundle:Admin:ul.childs.product.html.twig',
				array(
					'category' => $category
				)
			);
	...
	public function getCategoryAction()
	{
		...
		if (!empty($category)) {
			...
			return new AjaxSuccess(array(
				'html' => $this->renderView('AppMainBundle:Form:category.html.twig', array(
	...
	{
		$catRp = $this->getDoctrine()->getRepository('AppMainBundle:Product');
	...
	{
		$parentCategoryId = (int)$this->getRequest()->request->get('parentCategoryId');
		$parentCategory = $this->getDoctrine()->getRepository('AppMainBundle:Category')->find($parentCategoryId);
		if (!empty($parentCategory)) {
			$product = new Product();
			...
			$form = $this->createForm(new ProductType(), $product);
			return new AjaxSuccess(array(
				'html' => $this->renderView('AppMainBundle:Form:product.html.twig', array(
						'form' => $form->createView()
					)
				)
			));
	...
	public function searchUncatProductAction()
	{
		...
		if (!empty($condition)) {
			...
			$categoryProductsHtml = $this->renderView('AppMainBundle:Admin:search.product.html.twig',
				array(
					'products' => $products
				)
			);
